...
def _seed_chk_numbering(conn):
    ...
    already = conn.execute(sa.text(
        "SELECT id FROM numbering_schemes WHERE document_type_id = :dtid"
    ), {"dtid": dt[0]}).fetchone()
    if already:
        return
    conn.execute(sa.text(
...
def upgrade():
    ...
    if "vehicle_checklists" not in set(insp.get_table_names()):
        op.create_table(
            "checklist_templates",
