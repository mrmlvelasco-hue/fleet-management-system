...
parse_form_date()/parse_form_datetime() instead of calling
date.fromisoformat()/datetime.fromisoformat() directly.
"""
...
def parse_form_datetime(value, field_label: str,
                        required: bool = False) -> datetime | None:
    """Parse a YYYY-MM-DDTHH:MM (datetime-local input) form value, raising a
    friendly error on bad input. Returns None for blank/missing values
    ...
    try:
        ...
        raise DateFormatError(
            f"Invalid date/time format for {field_label}. Please use the "
            f"date/time picker to select a valid value.")
